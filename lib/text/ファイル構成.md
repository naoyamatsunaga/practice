...
lib/
  app/
    ...
    di/                     // 依存性注入（必要になったら）
  core/
    theme/                  // 必要になればテーマ拡張を配置（現状は MaterialApp のデフォルトテーマ）
...
- 定数など横断的なものは`core/`へ（テーマは現状 `main.dart` の `ThemeData()` / `ThemeData.dark()` を利用）
...
- `lib/home.dart`
  ...
  - 補足: DBアクセス（`watchTasks()`, `insertTask()`等）は`view_model/home/home_view_model.dart`へ移し、Viewは`ViewModel`を監視するだけにするのがMVVMの基本です
...
- リポジトリ（データ取得の抽象化）
  - 例: `TaskRepository` → `lib/data/repositories/task_repository.dart`
...
- ユースケース（任意）
  - 例: `AddTask` → `lib/domain/usecases/add_task.dart`
...
- ファイル名: スネークケース（例: `home_view_model.dart`, `task_repository.dart`）
- ViewModel: `<feature>_view_model.dart`（例: `home_view_model.dart`）
- Repository: `<resource>_repository.dart`（例: `task_repository.dart`）
